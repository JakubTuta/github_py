...
def open_file_dialog():
    global filepath
    filepath = filedialog.askopenfilename(title="Select a file", filetypes=((".png, .jpg", ("*.png", "*.jpg")), ("all files", "*.*")))
    open_smaller_image()
...
Label(text="Paste an image and resize it", font=title_font).grid(row=0, column=0, columnspan=3, padx=10, pady=10)

Label(text="Drop an image", font=main_font).grid(row=1, column=0, padx=10, pady=10, sticky=W)
...
Label(text="Upload an image", font=main_font).grid(row=3, column=0, padx=10, pady=10, sticky=W)
Button(text="Open a file", font=main_font, command=open_file_dialog).grid(row=4, column=0, padx=10, pady=10, ipadx=10, ipady=5)
...
Button(text="Ready", font=title_font, command=lambda: resize_and_save_image(WidthEntry.get(), HeightEntry.get())).grid(row=5, column=1, padx=10, pady=10, ipadx=30, ipady=10)
